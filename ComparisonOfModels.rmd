...
predictions_models = data.frame(pred_knn= predictions_KNNImputed,pred_rf =predictions_RFImputed, pred_svm = prediction_SVMImputed,pred_GBM = predictions_GBMImputed, pred_glm = predictions_GLMImputed,pred_Nnet= prediction_NNetImputed,actual= Adult_dummy_imputed$income) 


# glm for ensembling
ensemble_glmfull = train(actual~pred_glm+pred_GBM+pred_Nnet+pred_rf+pred_knn+pred_svm, predictions_models,method = "glm", family = "binomial", metric= "ROC", trControl=ctrl)
confusionMatrix(ensemble_glmfull$pred$pred[order(ensemble_glmfull$pred$rowIndex)], Adult_dummy_imputed[,"income"])

# Extra
ensemble_glm = train(actual~pred_glm+pred_GBM+pred_Nnet, predictions_models,method = "glm", family = "binomial", metric= "ROC", trControl=ctrl)
ensemble_rf = train(actual~pred_glm+pred_GBM+pred_Nnet, predictions_models,method = "rf", metric= "ROC", trControl=ctrl3)
# using svm, gbm and nnet 
ensemble_glm1 = train(actual~pred_glm+pred_GBM+pred_svm, predictions_models,method = "glm", family = "binomial", metric= "ROC", trControl=ctrl)
ensemble_rf1 = train(actual~pred_glm+pred_GBM+pred_svm, predictions_models,method = "rf", metric= "ROC", trControl=ctrl3)
ensemble_gbm = train(actual~pred_glm+pred_GBM+pred_svm, predictions_models,method = "gbm", metric= "ROC", trControl=ctrl3)
...
```{r}
weighted_majority = as.factor(ifelse
                              (predictions_models$pred_knn =='X0' & predictions_models$pred_rf =='X0', 'X0', ifelse(predictions_models$pred_knn =='X1' &  predictions_models$pred_rf =='X1', 'X1', ifelse( predictions_models$pred_Nnet =='X1', 'X1','X0'))))
cm_ensemble = confusionMatrix(weighted_majority, Adult_dummy_imputed$income, 'X1')
cm_ensemble
