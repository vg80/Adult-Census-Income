...
# kNN -smote
```{r}
grid = expand.grid(k=c(11,21,31,41,51))
kNNFit = train(Adult_dummy_del[,P1], Adult_dummy_del[,"income"], method = "knn", use.all = F,trControl = ctrl, metric = "ROC", tuneGrid = grid)
```
...
nbFit = train(Adult_dummy_del[,P1], Adult_dummy_del[,"income"], method = "nb", trControl = ctrl, metric = "ROC", tuneLength = 10)
...
# Control for under sample modelling
```{r}
set.seed(400)
ctrl_under = trainControl(method = "cv", 
                     number = 5, 
                     sampling = "under",
                     summaryFunction= twoClassSummary, 
                     classProbs=T,
...
# kNN -undersampling
```{r}
grid1 = expand.grid(k=c(11,21,31,41,51,100))
kNNFit_under = train(Adult_dummy_del[,P1], Adult_dummy_del[,"income"], method = "knn", use.all = F,trControl = ctrl, metric = "ROC", tuneGrid = grid1)
```
