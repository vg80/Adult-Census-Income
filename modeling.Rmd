...
# Comparing models
```{r}
Resamps= resamples(list(KNN=KNNFit, RF= RFFit, SVM = SVM_linearFit,GBM = GBMFit, GLM =GLMFit))
summary(Resamps)
theme1 <- trellis.par.get()
theme1$plot.symbol$col = rgb(.2, .2, .2, .4)
theme1$plot.symbol$pch = 16
...
top_models = data.frame(pred_knn = predictions_KNN,pred_rf =predictions_RF, pred_gbm = predictions_GBM, actual = Adult_dummy_del$income) 

#Ensembling

top_models$pred_majority = as.factor(ifelse(top_models$pred_gbm=='X0' & top_models$pred_knn=='X0','X0',ifelse(top_models$pred_gbm=='X0' & top_models$pred_rf=='X0','X0',ifelse(top_models$pred_knn=='X0' & top_models$pred_rf=='X0','X0','X1'))))
cm_ensemble = confusionMatrix(top_models$pred_majority, Adult_dummy_del$income)

# rf for ensembling
ensemble_rf = train(actual~., top_models,method = "rf", metric= "ROC", tunelength=3, trControl=ctrl)
